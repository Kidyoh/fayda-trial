  ...
  const calculateProgress = (materials: any[], studentId: string) => {
    ...
    const completedUnlockedMaterials = unlockedMaterials.filter((material) =>
      material?.StudentMaterial?.find(
        (item: any) => item.StudentId === studentId && item.Done === true,
      ),
    );

    return Math.round(
      (completedUnlockedMaterials.length / unlockedMaterials.length) * 100,
    );
  };

  // Move MaterialClicked above useEffect to avoid ReferenceError
  const MaterialClicked = React.useCallback(
    (materialId: any, materialtype: any, materialAccess: any) => {
      console.log("MaterialClicked called:", {
        materialId,
        materialtype,
        materialAccess,
      });
          ...
          if (jsonData[0]?.Courses?.materials?.length > 0) {
            ...
            if (!activeMaterialId || !activeMaterialtype) {
              ...
              if (unlockedMaterials.length > 0) {
                const firstUnlockedVideo = unlockedMaterials.find(
                  (material) => material.materialType === "video",
                );

                const selectedMaterial =
                  firstUnlockedVideo || unlockedMaterials[0];

                MaterialClicked(
                  selectedMaterial.id,
                  selectedMaterial.materialType,
                  selectedMaterial.Access,
                );
              }
            }
          }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#c7cc3f]/10 to-white pt-16">
      <div className="mx-auto px-4 md:px-6 lg:px-8 py-6 md:py-8">
        <CourseHeader
          ...
          courseDescription={
            data[0]?.Courses?.courseDescription ||
                ...
                <div className="flex flex-col items-center gap-4 text-center">
                  ...
                  <div className="text-gray-600 max-w-md">
                    <p className="text-sm text-gray-500">
                      Please wait while we load your course materials and
                      progress.
                    </p>
                  </div>
                </div>
