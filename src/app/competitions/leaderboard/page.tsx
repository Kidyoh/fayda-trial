...
export default function LeaderBoard() {
  ...
  const TopThreeCard = ({ student, rank }: { student: any; rank: number }) => {
    ...
    return (
      <div
        className={`relative p-6 rounded-lg border-2 bg-gradient-to-br ${colors[rank as keyof typeof colors]}`}
      >
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div
                className={`text-sm font-medium ${isCurrentUser ? "text-[#07705d]" : "text-gray-900"}`}
              >
                {student?.firstName} {student?.lastName}
                {isCurrentUser && (
                  <span className="ml-2 text-xs bg-[#07705d] text-white px-2 py-1 rounded-full">
                    You
                  </span>
                )}
              </div>
          ...
          <div className="text-right">
            <div
              className={`text-lg font-bold ${isCurrentUser ? "text-[#07705d]" : "text-[#bf8c13]"}`}
            >
              {parseFloat(student?.points).toFixed(1)}
            </div>
      ...
      <div className="relative z-10 py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h1 className="text-4xl font-sendako font-bold text-[#07705d] mb-2">
              Leaderboard
            </h1>
            <p className="text-gray-600">
              Compete with the best and rise to the top!
            </p>
          </div>
