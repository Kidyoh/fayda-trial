  ...
  async translatePage(targetLanguage: string): Promise<void> {
    try {
      ...
      const visibleTextNodes = textNodes.filter(node => {
        ...
        if (['script', 'style', 'noscript', 'code', 'pre'].includes(tagName)) return false;
      ...
      for (let i = 0; i < visibleTextNodes.length; i += batchSize) {
        ...
        const translatePromises = batch.map(async (node) => {
          ...
          if (text && text.length > 0 && text.length < 5000) { // Limit text length
            try {
              ...
              // Mark as translated
              if (node.parentElement) {
                node.parentElement.setAttribute('data-translated', targetLanguage);
